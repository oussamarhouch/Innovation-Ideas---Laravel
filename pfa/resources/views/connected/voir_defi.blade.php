...
<head>
    ...
    <link rel="stylesheet" href="{{ URL::asset('css/voir_defi.css') }}" />
    <script type="text/javascript" src="{{ URL::asset('js/voirdefi.js') }}"></script>
    ...
    <section id="about">
        <div class="about container">
          ...
          <div class="col-right">
            ...
            <div id="defititre">
              <h1 class="section-title" style="text-align: center">défi du <span>mois</span></h1>
              <p id="defi_mois">{{ $data[0]->defi }}</p>
            </div>
            <h1 class="section-title" id="section_title" style="text-align: center;"><span>Idées</span> publiées</h1><br><br><br>
            <div id="all_data">
            @foreach($idee as $key => $data)
            <table class="idea_table">
              <tr class=description_box>
                  <th class="titre_box"><span>Titre:</span> {{$data->titre}}</th>
                  <th class="username_box"><span>Publié par:</span> {{$data->username}}</th>
              ...
              <tr class=idea_container>
                <td class="idee_box" colspan="3">{{$data->idee}}</th>
              </tr>
            </table>
          @endforeach
        </div>
